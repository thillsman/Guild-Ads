...
export async function logAdRequest(
  ...
  if (error) {
    console.error('[sdk-api] failed to log ad request', error)
  }
}

export async function markLatestRequestClicked(
  supabase: SupabaseClient<Database>,
  ...
  if (requestError) {
    console.error('[sdk-api] failed to read latest ad request for click', requestError)
    return
  }
  ...
  if (updateError) {
    console.error('[sdk-api] failed to mark click on ad request', updateError)
  }
}
