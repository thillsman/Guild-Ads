...
declare
  v_closed_week record;
begin
  for v_closed_week in
    select distinct ws.week_start as closed_week_start
    from weekly_slots ws
    where ws.week_start < (
      timezone('utc', now())::date
      - extract(dow from timezone('utc', now())::date)::integer
    )
    order by closed_week_start asc
  loop
    perform run_weekly_earnings_accrual(v_closed_week.closed_week_start);
  end loop;
end;
