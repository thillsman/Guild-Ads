-- Refresh live network stats so publisher counts can bridge pre-serve-attempt data.
...
view_reach as (
  select
    uav.publisher_app_id as app_id,
    count(distinct uav.device_id_hash)::bigint as unique_users
  from unique_ad_views uav
  join params p on true
  where coalesce(uav.last_seen_at, uav.first_seen_at) >= p.window_start
  group by uav.publisher_app_id
),
publisher_app_reach as (
  select
    coalesce(sr.app_id, vr.app_id) as app_id,
    greatest(coalesce(sr.unique_users, 0), coalesce(vr.unique_users, 0))::bigint as unique_users
  from serve_reach sr
  full outer join view_reach vr on vr.app_id = sr.app_id
)
select
